...
namespace DuplicateFinderMulti.VM
{
  public class DummyWordService : IWordService
  {
    ...
    //    "Enable AWS Config and use it to automatically apply tags to newly launched EC2 instances using the Required-Tag managed rule.",
    //    "Set up an Amazon CloudWatch alarm to alert on untagged EC2 instances and create an Amazon SNS topic and add it to the alarm. Then write an AWS Lambda function to handle the tagging, and subscribe it to the topic",
    //    "Create an IAM policy that leverages the aws: RequestTag context key to apply the tags to the ec2: RunInstances action.",
    //    "Configure an Amazon CloudWatch Events rule to be triggered on instance state changes that will execute an AWS Lambda function to tag the instances",
    ...
    //    "Add AWS Cloud Trail logging for the S3 buckets",
    //    "Implement IAM policies to allow only the Storage team to create S3 buckets.",
    //    "Add the AWS Config managed rule S3_BUCKET LOGGING_ENABLED.",
    ...
    //    "Increase the CloudFront time to live (TTL) settings in the Cache Behavior Settings (It’s max-age, not ttl)",
    ...
    //    "Set up different metric filters for each team based on patterns and alerts Each alarm will notify the appropriate notification",
    ...
    //    "Modify the condition operator to include both Not IpAddress and IpAddress to prevent unauthorized access to the S3 bucket",
    //    "Modify the condition element from the IAM policy to aws: StringEquals instead of aws: Source Ip.",
    ...
    //    "Change Effect from Allow to Deny in the second statement of the policy to deny requests not from the source IP",
    ...
    //    "Create an AWS Lambda function to delete the 53 buckets if logging is not turned on",
    //    "https://docs.aws.amazon.com/config/latest/developerguide/s3-bucket-logging-enabled.html",
    //    "",
    //    "18.",
    //    "A company creates custom AMI images by launching new Amazon EC2 instances from an AWS CloudFormation template. It installs and configures necessary software through AWS OpsWorks and takes images of each EC2 instance. The process of installing and configuring software can take between 2 to 3 hours, but at times, the process stalls due to installation errors.",
    ...
    //    "Use AWS Trusted Advisor and enable email notification alerts for E02 disk space",
    ...
    //    "Use AWS Organizations to execute the Cloud Formation template in all accounts.",
    ...
    //    "Using the AWS Management Console, search for the user name in the Cloud Trail history Then filter by API and download the report in CSV format",
    //    "Use the Cloud Trail digest flies stored in the company's Amazon S3 bucket Then send the logs to Amazon QuickSight to create the report",
    //    "Locate the monthly reports that Cloud Trail sends that are emailed to the accounts Tool user Then forward the reports to the auditor using a secure channel",
    //    "Access the Cloud Trail logs stored in the Amazon S3 bucket tied to Cloud Trail. Use Amazon Athena to extract the information needed to generate the report",
